...
import com.util.framework.CommonKohl;
...
public class TestKohl extends CommonKohl {
	...
	public static String project = "Kohl";
	private final static Logger logger = Logger.getLogger(TestKohl.class);
	public static CommonKohl testMJ = new TestKohl();
...
//		String password = "gilberts";
		...
		TestKohl.rowIndex = Math.floorMod(TestKohl.acno, TestKohl.totalNoOfRows) + 1;

		System.out.println("Test Case test #" + TestKohl.rowIndex);

		cell1 = TestKohl.exportworkbook.getSheet(project).getRow(TestKohl.rowIndex).createCell(TestKohl.AcColStatus);
		cell1.setCellValue("");

		cell2 = TestKohl.exportworkbook.getSheet(project).getRow(TestKohl.rowIndex).createCell(TestKohl.AcColdetail);
		cell2.setCellValue("");
		...
		try {
			if (active.equalsIgnoreCase("Yes")) {
				if (purveyor.equalsIgnoreCase("Kohl")) {
					...
					System.out.println("No Restaurant available for Kohl");
				}
			} else {
